...
#        logger.addFilter(NoLoggingFilter())  # uncomment to decactivate logging for this function      
    ...
    def get_nounPhrases(self, nounhead):
        """
            ...
            It is an extension of function get_nounPhrase()
...
#        logger.addFilter(NoLoggingFilter())  # uncomment to decactivate logging for this function      
        ...
        for nmod,nmodchildren in nmod_conjs.items():
            ...
            for nmodchild in nmodchildren:
                ...
                logger.debug("conj-noun: " + conjnounPhrasetext)
...
#        logger.addFilter(NoLoggingFilter())  # uncomment to decactivate logging for this function      
    ...
    def get_conj_noun(self,nodeID):
        ...
        for successor in self.udgraph.successors(nodeID):
            if(self.udgraph[nodeID][successor]['relation']=='conj'):
                #conj_noun.append(self.get_nounPhrase(successor))
                conjnouns = self.get_nounPhrases(successor)
    ...
    def get_verb_code(self):
        ...
        def match_prep(path, prep_phrase):
            ### 17.07.31: uh, this is no longer doing anything...
            print("mp-entry")
            pass
...
#                    print(match)
        ...
        for triple in self.metadata['triplets']:
            ...
            if(len(verbtokens)>1):
                logger.debug("Token codes: " + str(codes))
                logger.debug("Token meanings: " + str(meanings))
                logger.debug("Token texts: " + str(matched_txts))
            ...
            if code != None:
                if len(code.split(":"))==2:
                    logger.debug("splitting: " + code)
                    active_code, passive_code = code.split(":")
                    if active_code == "":  # 17.07.28 pas: A construction of the form [:nnn] seems to indicate passive in PETR-2
                        if verb.passive == True:
                            verbcode = passive_code
                        else:
                            verbcode = active_code
                    else: # 17.07.28 pas: Constructions of the form {xxx:xxx] are paired codes in TABARI and PETR-1, whereas 
                          # this was interpreted as a active:passive pair in PETR-2 (see utilities.convert_code()) 
                        logger.debug("paired code: " + code)
                        verbcode = code
                        
                else:
                    verbcode = code
                logger.debug("verbcode: " + verbcode)
                logger.debug("verb: " + str(verb))
...
#        logger.addFilter(NoLoggingFilter())  # uncomment to decactivate logging for this function      
        ...
        for tripleID, triple in self.triplets.items():
            ...
            logger.debug("check event: " + str(triple))
...
#               logger.debug("source: "+ source.head+" code: "+(("#").join(source.meaning) if source.meaning != None else '-'))
        ...
        for key, value in self.events.items():
            logger.debug("event ==> " + key + " : " + str(value))
